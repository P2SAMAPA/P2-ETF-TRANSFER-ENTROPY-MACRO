...
from transfer_entropy import transfer_entropy_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    if macro_window.empty:
        return None
    raw_scores = transfer_entropy_score(ret_window, macro_window, lag=config.LAG, n_bins=config.N_BINS)
    norm_scores = normalize_scores(raw_scores)
    sorted_norm = sorted(norm_scores.items(), key=lambda x: x[1], reverse=True)
    top_etfs = [{"ticker": t, "te_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    results = {
        ...
        "selected_macros": config.SELECTED_MACROS,
    ...
    out_file = f"output/transfer_entropy_{timestamp}.json"
