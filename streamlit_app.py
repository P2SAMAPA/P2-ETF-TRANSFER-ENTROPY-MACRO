...
st.set_page_config(page_title="Transfer Entropy Network with Macro", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🔄 Transfer Entropy Network with Macro</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Information flow from macro variables to ETFs | Predictability signal</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Transfer Entropy")
...
st.sidebar.markdown(f"**Macros:** {', '.join(config.SELECTED_MACROS)}")
st.sidebar.markdown(f"**Bins:** {config.N_BINS} | **Lag:** {config.LAG}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'transfer_entropy_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Info flow score: {etf['te_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized TE"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized TE", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🔄 Top ETFs by Macro‑to‑ETF Transfer Entropy (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Transfer entropy** measures the directed information flow from a macro variable (e.g., VIX) to an ETF's future return.
        - We discretise both series and compute TE(macro -> ETF) = I(ETF_{t+lag}; macro_t | ETF_t).
        - The score for each ETF is the average TE across a set of selected macro variables.
        - High TE → the ETF's future returns are strongly influenced by macro – potentially more predictable.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the transfer entropy scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Transfer Entropy | Information flow from macro to ETFs")
